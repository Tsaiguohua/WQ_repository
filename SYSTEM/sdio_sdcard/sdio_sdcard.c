...
SD_Error SD_Init(void)
{
	...
	SD_Error errorstatus=SD_OK;	 
     ...
     GPIO_InitStructure.GPIO_Pin =GPIO_Pin_8|GPIO_Pin_12; 	//PC8,12复用功能输出，强制1-bit数据传输以避开PC10/PC11的UART4
     GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;//复用功能
     GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;//100M
     GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
     GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;//上拉
     GPIO_Init(GPIOC, &GPIO_InitStructure);// PC8,12复用功能输出
	...
	NVIC_Init(&NVIC_InitStructure);	//根据指定的参数初始化VIC寄存器、
 	...
 	if(errorstatus==SD_OK)errorstatus=SD_SelectDeselect((u32)(SDCardInfo.RCA<<16));//选中SD卡   
   	if(errorstatus==SD_OK)errorstatus=SD_EnableWideBusOperation(SDIO_BusWide_1b);	//由于PC10/PC11让给UART4，必须强制使用 1-bit 模式
